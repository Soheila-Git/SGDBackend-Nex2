...
class Filedbentity(Dbentity):
    ...
    def upload_file_to_s3(self, file, filename):
        ...
        if file_size < 5000000000:
            ...
            cwd = os.getcwd()
            s3_text = cwd+ '/src/large_s3_files.txt'
            with open(s3_text, 'a+') as s3_file:
                s3_path = self.sgdid + '/' + filename
                s3_file.write('path: ' + s3_path + ' | size: ' + str(file_size))
                s3_file.write("\n")

            #src_path = LOCAL_FILE_DIRECTORY + '/' + filename
            #dest_path = cwd + '/s3_data_files/' + filename
